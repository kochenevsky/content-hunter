...
import { Target, FileText, Video, Scissors, Share2, BarChart3 } from 'lucide-react'
...
export type HowItWorksSectionProps = {
  ...
  steps?: Array<{ icon?: string; title?: string; description?: string }> | null
}

export function HowItWorksSection({ title, subtitle, steps }: HowItWorksSectionProps = {}) {
  const stepList = steps?.length ? steps.map(s => ({
    Icon: getIcon(s.icon) ?? Target,
    title: s.title || '',
    description: s.description || '',
  })) : DEFAULT_STEPS.map(s => ({ Icon: s.icon, title: s.title, description: s.description }))
